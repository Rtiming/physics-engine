...
本节是那条通则的执行面：**54个案例**按**穿过引擎哪几层**分类，**不按案例数报成绩**。
...
| **`model_scene`→`dynamic_body`→`rigidbody`自由飞行（无接触）** | **1** | **`dynamic_model_scene_free_flight`**把命名质量属性、geometry frame内质心、初始世界系线速度/体系角速度装入13维COM刚体状态，用RK4推500步后再反算geometry位姿。它证明偏心几何绕COM转，不证明dynamic接触、约束或长时间守恒 |
