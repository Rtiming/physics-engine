...
本节是那条通则的执行面：**52个案例**按**穿过引擎哪几层**分类，**不按案例数报成绩**。
...
| **`model_snapshot`→`planned_motion`→`model_physics`输入装配（不碰求解器）** | **1** | **`model_motion_physics_binding`**把独立visual/collision资产、时间/无时间规划、多track位姿、源状态、累计送带、static/kinematic/dynamic所有权、显式排除的显示运动和虚拟process frame钉成一个内容寻址输入包；它验的是“哪个模型字节怎样进入虚拟物理”，不计算力 |
