...
本节是那条通则的执行面：**49个案例**按**穿过引擎哪几层**分类，**不按案例数报成绩**。
...
| **`tension_measurement`测力轮静力与安装映射（不碰`state`／`energies`／`solve`）** | **2** | **`tension_measuring_roll_resultant`**把两侧张力/方向变成轮上矢量合力；**`tension_measuring_roll_installation`**再把合力拆成gross/tare/net敏感轴力与显式支承载荷。两条都不解自由度，但它们验证的是**物理测量方程**，不是闭式计算器：被验模块会生成跨边界分层样点，而金标生成器不import它 |
