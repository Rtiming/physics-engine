...
状态：**现状索引+上位平台提案路由，2026-08-20更新本仓输入合同**。本页不冻结新接口，也不声明平台、
...
### 1.1 2026-08-20绕制偏差物理基础设施归属

用户进一步确认：绕制偏差的公共**物理**基础设施归本仓，不等待上位平台实现。
边界不是“所有模型代码都搬进来”，而是：

```text
通用模型快照 ──────┐
WII规划/运动状态adapter ─├─> 模型—运动—物理绑定 ─> physics-engine ─> WDS只读消费
本仓物理参数/现场标定 ───┘
```
...
[0099](decisions/0099_模型快照_规划运动与虚拟物理绑定归本仓_WII后适配_20260820.md)和
[plans/19](plans/19_绕制偏差物理基础设施与张力测量小场景计划_20260820.md)。
