...
def test_the_rolling_branch_matches_five_sevenths_g_sin_theta(oracles):
    ...
    **`a/(αR)`必须为1**——那是"无滑"这件事本身，不是它的推论。
    ...
    measured = _roll(entry.inputs["friction_coefficient"], entry.inputs)
    _check(entry, measured)
    assert measured["sliding_flag"] == 0.0


def test_the_sliding_branch_matches_the_saturated_closed_form(oracles):
...
def test_the_two_branches_are_told_apart_by_the_ratio_not_by_the_formula(oracles):
...
def test_the_normal_penalty_makes_exactly_zero_torque_on_a_sphere(oracles):
