...
def test_tension_measurement_sample_is_registered_as_draft():
    facet = ENGINE_REGISTRY.get(TENSION_MEASUREMENT_SAMPLE_FACET)
    assert TENSION_MEASUREMENT_SAMPLE_VERSION == "0.1"
...
def test_red_untested_or_wrong_major_measurement_versions_are_rejected(version):
    with pytest.raises(FacetError):
        ENGINE_REGISTRY.assert_reader_compatible(TENSION_MEASUREMENT_SAMPLE_FACET, version)


def test_measurement_facet_constants_are_exported():
    from physics_engine import engine_facets

    assert "TENSION_MEASUREMENT_SAMPLE_FACET" in engine_facets.__all__
    assert "TENSION_MEASUREMENT_SAMPLE_VERSION" in engine_facets.__all__
