...
PHYSICS_DOMAINS: dict[str, tuple[str, ...]] = {
    ...
    #: 这里就是那一行，门①②③⑤一个字都没改。research/08第七节预判"电磁大概率
    #: 会触发要改门"，**实测没有**：它要改的是spec/12的状态形制与spec/14的材料
    #: 长度制，而互感与Norris闭式两条路都不碰（不进`State`、不读材料记录）。
    #: 触发条件：`Jc`/`Ic`真进材料记录那天它会import `materials`（基座），
    #: 门③届时起作用；若哪天要读`state`里的电流自由度，门①当场红。
