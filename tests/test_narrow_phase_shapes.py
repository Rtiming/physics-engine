...
#: 跨平台那一半允许的最大ulp差。**这个数是量出来的，而且量错过一次。**
#:
#: 2026-08-19实测同一棵树在macOS arm64与Linux x86-64上，34对里有2对不同：
#:
#: * `0x1.ca98be8f7ff90p+0` 对 `…7ff80p+0` —— **16 ulp**（rel 2.00e-15）
#: * `0x1.8089984a00f89p+4` 对 `…00f88p+4` —— **1 ulp**（rel 2.22e-16）
#:
#: **第一版把这两条都读成了"差1 ulp"并据此取了余量2，当场红。**
#: 病根是看十六进制看漏了位权：`float.hex()`一个十六进制位是**4个比特**，
#: 所以"最后一位差1"是16 ulp不是1 ulp。**这条注记留着，因为下一个人会照着读同一串字符。**
#:
#: 取32：覆盖实测最坏的16并留一倍余量。**它不是"随便给的宽"**——
#: 32 ulp在这个量级上是rel约4e-15，仍比这些接触量本身该有的精度紧几个数量级；
#: **超过32就不再是"平台末位差"，是真的动了数**（配必红）。
#: 成因是`x ** 0.5`与libm在末位上的实现差——本轨自己在20万个正数里量到过277个差1 ulp，
#: 而`penetration_mm`是若干次这类运算之后的结果，末位差会攒。
_CROSS_PLATFORM_ULP_BUDGET = 32
