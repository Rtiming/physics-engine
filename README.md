...
| ⑤平面带材绕十几匝 | 矩形截面逐点弹塑性已接入**一个WDS运动学三节点站点**的全局残差/Hessian与回弹；WDS main已有默认关闭、由R250 v002显式启用的正式0.6.0线弹性单站接缝（`sections`+`section_beam`，0059—0061） | ❌ | 仅easy-axis单站、固定轴向应变；正式接缝不等于十几匝端到端完成，仍无引擎侧整杆多站、自接触、半径生长、喂料、宽度运动学、压扁/翘曲/扭转 |
...
三节点站点的节点/扭角运动学生成后装入全局残差与Hessian；仍无轴向N联立、多站整杆、
独立截面场、压扁/翘曲/扭转；WDS main已有默认关闭、只由R250 v002显式启用的正式
0.6.0线弹性单站接缝）、
...
没有网格、没有复数场、没有FFT。截面积分只接了**一个easy-axis三节点站点**，固定
轴向应变；没有整杆多站、轴向N联立、hard-axis、剪切、扭转、翘曲、压扁、二维任意
截面或电磁截面场自由度。
...
| 3 | 存量迁移：消费方把既有引擎层改为依赖本仓，各自附逐字节不变证据；操作单在`docs/migration/` | **进行中**——`winding-deviation-sim`已从0.4.0基础消费推进到正式0.6.0线弹性单站接缝（main `f88986f7...`，既有14案双版本门完成），但其余契约/storage迁移与完整力学域仍未完成；`fts-digital-twin`闸门0已过（其ADR 0036）但引擎层未切换。逐行事实以[spec/90消费方登记](docs/spec/90_消费方登记.md)为正本 |
