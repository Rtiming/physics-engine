...
能力位清单的当前机械计数是**11/42**（其中穿过物理机械的done位4/11）；
...
| ③不规则体落漏斗 | 刚体自由飞行；**10球平面漏斗最小组合已跑通**（重力+球/壁罚接触+线性dashpot+耗散账） | 🔶部分 | 仍是解析球和三个平面；接触对全量预声明，**动态检测没有驱动响应**；无网格窄相、转动、摩擦与实验标定 |
...
**接触**（🔶罚法向、摩擦历史、法向dashpot与10球最小组合已落；仍缺网格窄相、
动态检测—响应接线、载荷步回退和转动自由度）、**耦合**
...
没有网格、没有复数场、没有FFT。
**接触仍缺四样**：网格窄相（圆柱/盒/网格族）、动态接触检测到响应的接线、
载荷控制失败后的真回退、以及**接触体的转动自由度**——今天是质点+半径、**没有力矩**，
...
- `docs/decisions/` — 本仓自己的决策记录（0001—0057，编号有缺口见0049第八节）
