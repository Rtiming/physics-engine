...
| ⑤平面带材绕十几匝 | **矩形截面局部弹塑性分布与自由回弹已跑通**（`sections`，0059） | ❌ | 尚未接全局带材/梁节点；无自接触、半径生长、喂料、宽度方向运动学、压扁/翘曲/扭转 |
...
25个案例不是一类东西。走完`state`→`energies`→`solve`整条路的**6条**
...
其余6条验几何查询、资产治理与模型生成。**6+1+3+3+1+5+6 = 25。**
...
**体积与厚度**（🔶0059已落矩形纤维截面的局部本构/分布切片：两个广义运动学坐标、
逐点弹塑性应力与显式历史；仍无独立截面场、压扁/翘曲/扭转及全局梁装配）、
...
没有网格、没有复数场、没有FFT。截面积分尚未接入全局梁单元，没有剪切、扭转、
翘曲、压扁、二维任意截面或电磁截面场自由度。
...
- `docs/decisions/` — 本仓自己的决策记录（0001—0059，编号有缺口见0049第八节）
