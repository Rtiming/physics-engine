...
| ⑥完整放线—导向—张力—收线 | 场景装配与接触对声明（`scene.Scene`）、位姿时间线（`motion`）、接触力（`contact`）；**带材绕导向轮已可算**（0062：有限长圆柱侧面罚接触＋多槽位同时滑移，`capstan_tension_ratio`用真机导轮尺寸对精确离散绞盘式） | ❌ | 带材在接触里仍是**中心线上的点、没有宽度**，所以碰不到法兰；轮**固定**不自旋；接触对是声明的不是检测的；`Actuator.apply`与`Sensor.read`的物理仍未实现 |

**六个场景都已有某些底层能力或理想化片段，但没有一个端到端完成。**

### 案例穿过引擎哪几层

27个案例不是一类东西。走完`state`→`energies`→`solve`整条路的**7条**
（两条悬臂+Euler屈曲+**四条接触**：静置阈值、历史迟滞、多体金字塔、绞盘张力比）；
...
其余6条验几何查询、资产治理与模型生成。**7+1+1+3+3+1+5+6 = 27。**
