...
| ⑤平面带材绕十几匝 | 矩形截面逐点弹塑性已接入**一个WDS运动学三节点站点**的全局残差/Hessian与回弹（`sections`+`section_beam`，0059/0060） | ❌ | 仅easy-axis单站、固定轴向应变；WDS尚未实际采纳，无整杆多站、自接触、半径生长、喂料、宽度运动学、压扁/翘曲/扭转 |
...
（`section_beam`，决策0060；这是源级兼容，不是WDS已迁移）；
...
- `docs/decisions/` — 本仓自己的决策记录（0001—0060，编号有缺口见0049第八节）
