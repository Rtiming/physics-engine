...
| ⑤平面带材绕十几匝 | 矩形截面逐点弹塑性已接入**一个WDS运动学三节点站点**的全局残差/Hessian与回弹；WDS独立分支另有默认关闭的线弹性单站采纳候选（`sections`+`section_beam`，0059—0061） | ❌ | 仅easy-axis单站、固定轴向应变；候选尚未发布/合入/过master门，无整杆多站、自接触、半径生长、喂料、宽度运动学、压扁/翘曲/扭转 |
...
独立截面场、压扁/翘曲/扭转；WDS仅有默认关闭的本地采纳候选，尚未正式发布/合入）、
...
（`section_beam`，决策0060；0061另有WDS默认关闭的本地采纳候选，不等于正式迁移完成）；
