...
#: 0.1弧度的相位误差对应``|exp(i phi) - 1| <= 0.1``，即振幅的相对偏差上界10%，
#: 而强度是它的平方项——实测（`test_the_declared_edge_phase_bound_is_measured_not_guessed`）
#: 在门槛处主瓣内强度相对偏差**7.9e-3**。菲涅耳数上界由它折算：
#: ``N_F <= 0.1/pi = 0.0318``（`fraunhofer_max_fresnel_number`）。
...
def rectangular_aperture(
    *,
    ...
    边界取**闭区间**``|x| <= a``：半宽恰好落在采样点上时那一点**算在孔内**。
    ...
    return ComplexField2D.from_function(
        ...
        lambda row, column: complex(
            1.0 if abs(xs[column]) <= half_x and abs(ys[row]) <= half_y else 0.0, 0.0
        ),
    )
