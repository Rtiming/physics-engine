...
- `physics_engine.rotation` —— **准静态路径上的转动自由度**（力学域，决策0079）：
  SO(3)指数映射的局部图（三个无约束参数`θ`）、它的一/二阶导、重输运；
  质心块+转动块的定长布局；两端都可自由的物质点粘着弹簧与它的**转动增量项**
  （接触力对质心取矩，Hessian含几何刚度）；`PointLoad`的转动对应物`AppliedMoment`。
  与`rigidbody`的四元数是同一个群的两种坐标：**准静态用局部图、动态用四元数**（0074第四节）
- `physics_engine.contact_dynamics` —— **接触力矩装配层**（力学域，plans/16的M1）：
  把接触的几何量接到`rigidbody`的两个回调上。`tau = r x F` **只在这里出现一次**——
  research/17第二节实测同行（Bullet第537行、Fang & Negrut附录A.2）都把它显式算出来，
  而喂一个裸力矩数会让线动量与角动量两条账各自看着都对、耦合却没被验。
  杆臂取**几何半径**、不折算渗透量（与`contact/penalty.py`的`radii_sum`同源；
  research/17第一节：否则相对速度里会多出量级 `delta*omega` 的伪滑移）。
  **不做滚动阻力**——它在所有主流同行那里都是显式声明的第二套力偶（research/17第三节）
...
- `physics_engine.model_scene` —— P3.1将已验证模型输入装配为`FinalizedScene`、
  时间/规划尺度运行面、虚拟frame、显式碰撞候选与dynamic状态集位姿
- `physics_engine.dynamic_body` —— geometry frame内质心/惯量与13维COM刚体状态的
  初始化和可逆几何位姿回读（决策0101）
- `physics_engine.dynamic_contact` —— 两dynamic解析球的单候选检测—法向旋量—
  26维耦合推进：每个RK4子阶段经`model_scene`反算几何，从CollisionEvent
  法向/见证点装作用反作用力与两侧`r×F`（决策0103；无摩擦历史/滚动阻力/非球）
...
进入既有接触项。0103又让一对解析球的两个dynamic刚体经窄相见证点获得作用反作用力
和两侧体系力矩，在同一26维RK4系统中推进。**仍缺**：网格窄相、空间索引、
解析平面统一动态接线、非球dynamic响应、摩擦历史、滚动阻力和载荷控制失败后的真回退。
